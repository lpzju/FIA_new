...
def load_image(images_list,image_size,batch_size):
    images=[]
    filenames=[]
...
def main(tid):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            count=0
            for images,names in utils.load_image(FLAGS.input_dir, FLAGS.image_size,FLAGS.batch_size):
                ...
                with open(FLAGS.output_dir + '/labels.json', 'a+') as f:
                    ...
                    for _ in range(len(label_name)):
                        logs["img_" + str(_ + 1)] = {}
                        logs["img_" + str(_ + 1)]["label_name"] = label_name[_]
                    json.dump(logs, f)
                ...
                for i in range(num_iter):
                    ...
                    # optimization
                    images_adv, grad_np, amplification_np, features=sess.run([adv_input_update, noise, amplification_update,opt_operations[0]],
                    ...
                    # save feature map
                    features_tmp=np.sum(features,axis=-1)[FLAGS.batch_size:]
                    for _ in range(len(names)):
                        save_name=os.path.splitext(names[_])[0]+'_feature_'+str(i+1)
                        dpi=300
                        plt.imshow(features_tmp[_])
                        plt.axis('off')
                        plt.gca().xaxis.set_major_locator(plt.NullLocator())
                        plt.gca().yaxis.set_major_locator(plt.NullLocator())
                        # plt.savefig(FLAGS.output_dir+save_name+'.jpg',bbox_inches='tight',pad_inches=0.0,dpi=dpi)
                        plt.savefig(FLAGS.output_dir + save_name + '.png', bbox_inches='tight', pad_inches=0.0, dpi=dpi)
                        plt.close()
